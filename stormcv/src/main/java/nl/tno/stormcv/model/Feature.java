...
/**
 * This {@link CVParticle} implementation represents a single feature calculated
 * for {@link Frame} and has the following fields:
 * <ul>
 * <li>name: the name of the feature like 'SIFT', 'SURF', 'HOG' etc</li>
 * <li>duration: the duration of the feature in case it describes a temporal
 * aspect of multiple frames</li>
 * <li>sparseDescriptors: a list with {@link Descriptor} objects used to
 * described sparse features like SIFT</li>
 * <li>denseDescriptors: a three dimensional float array much like the OpenCV
 * Mat object which can be used to represent dense features like dense Optical
 * Flow</li>
 * </ul>
 * It is not always clear how a specific descriptor should be stored and it is
 * typically up to the characteristics of the topology and context what is the
 * best way to go.
 *
...
public class Feature extends CVParticle {
    ...
    public Feature deepCopy() {
        ...
        List<Descriptor> sparseCopy = new ArrayList<>(this.sparseDescriptors.size());
        for (Descriptor d : sparseDescriptors) {
            sparseCopy.add(d.deepCopy());
        }

        Feature copyFeature = new Feature(this.getStreamId(), this.getSequenceNr(), this.getName(), this.getDuration(),
